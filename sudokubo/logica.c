...
int movimiento_valido(const t_tablero *t, int fila, int col, int valor) {
    ...
    for (j = 0; j < tam; j = j + 1) {
        if (j != col) {
            v = char_a_valor(t->c[fila][j].car, tam);
            if (v == valor) return 0;
        }
    }
    ...
    for (i = 0; i < tam; i = i + 1) {
        if (i != fila) {
            v = char_a_valor(t->c[i][col].car, tam);
            if (v == valor) return 0;
        }
    }
    ...
    for (i = ini_f; i < ini_f + sub; i = i + 1) {
        for (j = ini_c; j < ini_c + sub; j = j + 1) {
            if (!(i == fila && j == col)) {
                v = char_a_valor(t->c[i][j].car, tam);
                if (v == valor) return 0;
            }
        }
    }
...
int sudoku_finalizado(const t_tablero *t) {
    ...
    for (i = 0; i < tam; i = i + 1) {
        ...
        for (j = 0; j < tam; j = j + 1) {
            vf = char_a_valor(t->c[i][j].car, tam);
            vc = char_a_valor(t->c[j][i].car, tam);
